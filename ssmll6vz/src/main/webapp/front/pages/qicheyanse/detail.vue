...
<script>
	export default {
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','qicheyanse');
			...
			onAcrossTap(tableName){
     			uni.setStorageSync('crossTable','qicheyanse');
				uni.setStorageSync(`crossObj`, this.detail);
				this.$utils.jump(`../${tableName}/add-or-update?cross=true`);
			},
			// 获取详情
			async init(){
				let res = await this.$api.info('qicheyanse', this.id);
			...
			// 添加评论
			onCommentTap() {
				this.$utils.jump(`../discussqicheyanse/add-or-update?refid=${this.id}`)
			},
			onSHTap() {
			...
			async onFinishTap() {
				...
				await this.$api.update('qicheyanse', this.detail);
				this.$utils.msg('审核成功');
